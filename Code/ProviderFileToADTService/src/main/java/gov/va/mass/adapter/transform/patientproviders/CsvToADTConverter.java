...
public class CsvToADTConverter
{
	...
	private void createADTMessageFromElements(String[] elements)
	{ // , Terser terser) {
		...
		if (!lastlinepatientien.equals(curlinepatientien))
		{ // if a new patient ien is being sent, then fire off the
			// old message
			sendMessageToReadyToSendToEpicQueue();
		...
		for (int i = 0; i < elements.length; ++i)
		{
			...
			try
			{
				if (value != null)
				{
					if (i < 5 && providercount == -1)
					{ // Upto patient elements create PID only when it is a new
						// patient.
					...
					{ // rest of the elements will be handled by the
	...
	private void sendMessageToReadyToSendToEpicQueue()
	{
		...
		catch (Exception e)
		{
			logger.error("Problem occured when accessing the queue. Name may be wrong "
					+ "or queue may not exist. Unable to send messages. Blocked at msg" + " with patient id " + lastlinepatientien
					+ e.getMessage());
		}
	...
	private void addProvidersToTerser(String[] elements)
	{ // , Terser terser) {
